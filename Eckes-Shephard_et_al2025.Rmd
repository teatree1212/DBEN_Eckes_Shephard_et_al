...
i=1
for(model_name in models_avail){
     boxplot(at=i,collect_turnover_FIN[[model_name]],col = get_model_colour(model_name),ylim=c(0,400), axes=FALSE, outcol = get_model_colour(model_name) ,add=TRUE)
...
i=1
for(model_name in models_avail){
     boxplot(at=i,collect_turnover_BIA[[model_name]],col = get_model_colour(model_name),ylim=c(0,160), axes=FALSE, outcol = get_model_colour(model_name) ,add=TRUE)
     i=i+1 # next model position
}
rm_dot <- c(2,4,8) # some models have large boxplots, so don't need dots for identification by colour
...
bp_stats <- boxplot(BCI_Turnover$turnover_time,plot= FALSE)
lower <- bp_stats$stats[1] #lower whisker
upper <- bp_stats$stats[5] #upper whisker
...
i=1
for(model_name in models_avail){
     boxplot(at=i,collect_turnover_BCI[[model_name]],col = get_model_colour(model_name),ylim=c(0,140), axes=FALSE, outcol = get_model_colour(model_name) ,add=TRUE)
     i=i+1 # next model position
}
...
points(c(1:10)[-rm_dot],boxplot_md[-rm_dot] ,col = get_model_colour(models_avail[-rm_dot]),pch=model_pch,cex=1.4)
boxplot(at=10,c(BCI_Turnover$turnover_time),ylim=c(0,1.7), axes=FALSE,add = TRUE,col="gray34") #observations
#add number of samples:
text(x = 10, y = upper + 0.1 * upper, labels = paste("n =", bp_stats$n[1]), cex = 0.8)
