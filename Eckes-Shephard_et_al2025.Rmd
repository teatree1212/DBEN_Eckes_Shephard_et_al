...
#base.path <- "/Users/annemarie/Library/CloudStorage/OneDrive-LundUniversity/1_TreeMort_onedrive/3_Dissemination/2_Publications/2025_Eckes_Shephard_et_al/analysis/"
...
source(paste0("DBEN_helper_functions_clean.R"))
...
# sampling of random numbers for forcing data before running the models (see supplementary materials 3- simulation protocol):
set.seed(143)
...
png(file=paste0(Figs_dir,simdate,"/S2_woody_carbon_budget.png"),width=215,height=340,units="mm",res =300)
...
pdf(file=paste0(Figs_dir,"Supplementary_materials_6_Benchmark_regrowth_per_model.pdf"))
...
# Table A2 to highlight the large difference in Carbon content at the same timing of recovery, relative to each model's equilibrium value.
...
pdf(file=paste0(Figs_dir,simdate,"/Supplementary_materials_7_Benchmark_stand_structure_per_model.pdf"))
...
png(filename=paste0(Figs_dir,"/SXX_Benchmark_stand_structure_LPJ-GUESS.png"),width=150,height=75,units = "mm",res=360)
...
pdf(file=paste0(Figs_dir,simdate,"/SXX_forest_phase_identification.pdf"),width=8,height=8)
...
#now smoothing the rates:
  ...
  mort_rate_means <- create_rolling_means(mort_rate,k = k,total_only = TRUE,
                                                align = "left")@data$Total
########################################################
bitmap(file=paste0(Figs_dir,"SXX_mortality_rates_FIN.png"),width=180,height=180,units="mm",res =380,type="png16m")
...
########################################################
bitmap(file=paste0(Figs_dir,"SXX_mortality_rates_BIA.png"),width=180,height=180,units="mm",res =380,type="png16m")
...
########################################################
bitmap(file=paste0(Figs_dir,"SXXmortality_rates_BCI.png"),width=180,height=180,units="mm",res =1300,type="png16m")
...
for(model_name in models_avail){
  for(site in sites){
    FULL[which(FULL$sites==site),][[model_name]][3] <- min(t(growth[[site]][model_name])[1:xmax])# record min value within 30 yrs
    FULL[which(FULL$sites==site),][[model_name]][4] <- max(t(growth[[site]][model_name])[1:xmax])#record max value within 30 yrs
    FULL[which(FULL$sites==site),][[model_name]][5] <- min(t(mort[[site]][model_name])[1:xmax])  # record min value within 30 yrs
    FULL[which(FULL$sites==site),][[model_name]][6] <- max(t(mort[[site]][model_name])[1:xmax])  #record max value within 30 yrs
...
create_gobj <- function(model_out, y.lim = NULL,layers=NULL){
  if(model_out@source@name ==  "LPJGUESS BCI - P0"){ # enhance title, so that the reader understands what layers are currently plotted
    ...
    p_out <- plotTemporal(model_out, legend.position = "bottom", layers=layers,y.lim, title = paste(model_out@source@name," \n", layers),subtitle = NULL,
                          text.multiplier = 0.8,x.label = "Years")
...
bitmap(file=paste0(Figs_dir,simdate,"/SXX_growth_rates_and_succession.png"),width=80,height=120,units="mm",res =480,type="png16m")

par(mfcol =c(1,2), oma=c(4.2,0,0.1,1.3),mar=c(0,3,0,0.5))
...
#bitmap(file=paste0(Figs_dir,simdate,"/SXX_all_models_biomass_succession.png"),width=80,height=120,units="mm",res =480,type="png16m")
pdf(file=paste0(Figs_dir,"SXX_all_models_biomass_succession.pdf"))
...
```{r turnover_times_}
bitmap(file=paste0(Figs_dir,"/SXX_regrowth_speed_biomass_equilibrium.png"),width=80,height=120,units="mm",res =480,type="png16m")
...
png( paste0(Figs_dir,"SXX_met_analysis.png"), width = 90, height = 180 ,units = "mm",res = 900)  # Adjust width, height, and resolution as needed
