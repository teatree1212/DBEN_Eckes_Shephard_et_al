...
library(dplyr)
library(maps)
library(sf) # no available after R 4.2.0. Alternative: sf
...
get_output_JULESRED <- function(site,run,var,file.dir,co2_levels){
  ...
  if(var == "cmort" | var == "cmort_age" |
     var == "cmort_other" | var == "cmort_greff" ) {
    #JULESRED must have changed these to -yr in post-processing now (20.07.2023)
    var.to.plot@quant@units <- "kgC m-2 yr-1"
  }
...
get_model_run_number <- function(model_name,protocol_run_name){
  ...
  df[df$Mnames =="SEIB-DGVM",2:9]        <- c("P0","P1","P2","P3","P4","P5","P6","P7") # 
  df[df$Mnames =="ORCHIDEE",2:9]        <- c("P0","P0","P0","P0","P0","P0","P0","P0") # 
