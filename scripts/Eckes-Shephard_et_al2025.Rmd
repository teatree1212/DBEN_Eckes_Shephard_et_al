---
title: "Analysis code for the publication Demography, Dynamics and Data: Growing Confidence for Simulating Changes in the World's Forests"
...
png(file=paste0(Figs_dir,"/FigS1_woody_carbon_budget.png"),width=215,height=340,units="mm",res =300)
...
png(filename = paste0(Figs_dir,simdate,"/Figure_1.png"),width=180,height=240,units = "mm",res=1000)
...
png(filename=paste0(Figs_dir,simdate,"/Figure_4.png"),width=150,height=150,units = "mm",res=960)
#bitmap(file=paste0(Figs_dir,simdate,"/Figure_4.png"),width=80,height=80,units="mm",res =960,type="png16m")
 ...
 for(model_name in models_avail){
  ...
  if(biome== "Temperate"| biome =="Boreal"){ #all values used, median used
    arrows(stand_structure_obs[which(stand_structure_obs$site == site),]$dbh_classes_num,
           ...
           log10(stand_structure_obs[which(stand_structure_obs$site == site),]$nstem_size_upper_ha.1),
            ...
            col= get_biome_colour(site,black_only=TRUE),lwd=0.6 )
    }
  if(biome =="Tropics"){ # again, bootstrapping samples from 50 ha,plot, reporting median and 95% CI
     arrows(stand_structure_obs[which(stand_structure_obs$site == site),]$dbh_classes_num,
           ...
           log10(stand_structure_obs[which(stand_structure_obs$site == site),]$nstem_size_upper_ha.1),
            length = 0.01, angle = 90, code = 3,
            col= get_biome_colour(site,black_only=TRUE),lwd=0.6 ,lty=4)
  }
...
mtext(side=2,expression(C["wood,AG"]~(kgC~m^-2)),line=-24,outer=TRUE)
...
pdf(file=paste0(Figs_dir,"/NotesS3_Benchmark_stand_structure_per_model.pdf"))
...
pdf(file=paste0(Figs_dir,"/FigS11_Supplementary_materials_Benchmark_self_thinning_all_models_point_selection_methods_3plots.pdf"))
...
png(filename=paste0(Figs_dir,simdate,"/Figure_3.png"),width=80,height=90,units = "mm",res=1300)
#bitmap(file=paste0(Figs_dir,simdate,"/Figure_3.png"),width=80,height=90,units="mm",res =1300,type="png16m")
...
#png(file=paste0(Figs_dir,simdate,"/Figure_5.png"),width=215,height=340,units="mm",res =300)
png(file=paste0(Figs_dir,simdate,"/Figure_5.png"),width=180, height=180, units="mm",res =1000)
#bitmap(file=paste0(Figs_dir,simdate,"/Figure_5.png"),width=80,height=80,units="mm",res =1300,type="png16m")
...
#tiff(file=paste0(Figs_dir,simdate,"/Figure_5.tiff"),width=160,height=200,units="mm",res =1300)

#tiff("boxplot_highres.tiff", width = 180, height = 240, units = "mm", res = 600)
...
boxplot_data <- collect_turnover_FIN
# add observations:
boxplot_data$Observations <- FIN_Turnover$turnover_time
...
plot.new()
plot.window(xlim = c(0.5, n_models + 1.5), ylim = c(ymin, ymax))

# Draw grey polygon first (observational range)
bp_stats <- boxplot(boxplot_data$Observations, plot = FALSE)
lower <- bp_stats$stats[1]
upper <- bp_stats$stats[5]

x_coords <- c(0.5, n_models + 0.5, n_models + 0.5, 0.5)
...
#Add the boxplots on top 
i=1
for(model_name in models_avail){
  boxplot(at=i,boxplot_data[[model_name]],col = get_model_colour(model_name),ylim=c(0,140), axes=FALSE, outcol = get_model_colour(model_name) ,add=TRUE)
  i=i+1 # next model position
}

rm_dot <- c(3,4,5,8) # some models have large boxplots, so don't need dots for identification by colour
boxplot_md <- sapply(boxplot_data[-10],median) # get boxplot horizontal line (= mean) for plotting of models that don't have a wide spread
points(c(1:9)[-rm_dot],boxplot_md[-rm_dot] ,col = get_model_colour(models_avail[-rm_dot]),pch=model_pch,cex=1.4)
boxplot(at=10,na.omit(c(boxplot_data$Observations)),ylim=c(0,1.7), axes=FALSE,add = TRUE,col="gray34") #observations
...
boxplot_data <- collect_turnover_BIA
# add observations:
boxplot_data$Observations <- BIA_Turnover$turnover_time
...
boxplot_data <- collect_turnover_BCI
# add observations
boxplot_data$Observations <- BCI_Turnover$turnover_time
...
rm_dot <- c(4,8) # some models have large boxplots, so don't need dots for identification by colour
boxplot_md <- sapply(boxplot_data[-10],median) # get boxplot horizontal line (= mean) for plotting of models that don't have a wide spread
points(c(1:9)[-rm_dot],boxplot_md[-rm_dot] ,col = get_model_colour(models_avail[-rm_dot]),pch=model_pch,cex=1.4)
boxplot(at=10,na.omit(c(boxplot_data$Observations)),ylim=c(0,1.7), axes=FALSE,add = TRUE,col="gray34") #observations
#add number of samples:
...
boxplot_data <- collect_boxplot_growth_FIN
# add observations
boxplot_data$Observations <- FIN$WBgrowth_kgC_m2_yr
...
boxplot_data <- collect_boxplot_growth_BIA
# add observations
boxplot_data$Observations <- BIA$WBgrowth_kgC_m2_yr
...
#add number of samples:
text(x = 10, y = upper + 0.1 * upper, labels = paste("n =", bp_stats$n[1]), cex = 0.8)
mtext(side=2,expression("eq. WBgrowth flux (kgCm"^-2~"yr"^-1~")"),line=1)
axis(1, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=F)
axis(side=2,tick = F,line = -0.7)#,at=c(420,430,440,450))
axis(2, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=F)
box()
...
boxplot_data <- collect_boxplot_growth_BCI
# add observations
boxplot_data$Observations <- BCI$WBgrowth_kgC_m2_yr
...
legend(legend = c("BiomeES","BiomeEP","CABLE-POP","FATES","JULES-RED","LPJ-GUESS","EDv3","SEIB-DGVM","ORCHIDEE","Observations"),
       ...
       "bottomright",ncol=6,box.lwd = 0,inset = c(0,
                                                  0),cex=1)

mtext(outer =TRUE, "FIN", side=3, adj=0.01, line=-4)
mtext(outer =TRUE, "BIA", side=3, adj=0.01, line=-16)
mtext(outer =TRUE, "BCI", side=3, adj=0.01, line=-34)
...
#pdf(file=paste0(Figs_dir,"/Notes_S2_prep_forest_phase_identification.pdf"),width=8,height=8)
...
#now smoothing the rates:
  ...
  mort_rate_means <- create_rolling_means(mort_rate,k = k,total_only = TRUE,
                                                align = "left")@data$Total
png(filename=paste0(Figs_dir,"FigS15_mortality_rates_FIN.png"),width=180,height=180,units = "mm",res=380)
...
png(filename=paste0(Figs_dir,"FigS16_mortality_rates_BIA.png"),width=180,height=180,units = "mm",res=380)
...
png(filename=paste0(Figs_dir,"FigS17_mortality_rates_BCI.png"),width=180,height=180,units = "mm",res=1300)
...
```{r growth_rate_kgCm2yr}
#The figures created in this chunk are not presented in publication in this form.
#the code must be executed for the below chunk, so that the data is plotted in Figure 2.
...
png(filename=paste0(Figs_dir,simdate,"/Figure_2.png"),width=80,height=120,units = "mm",res=600)
#bitmap(file=paste0(Figs_dir,simdate,"/Figure_2.png"),width=80,height=120,units="mm",res =1000,type="png16m")
...
par(mfcol =c(3,2), oma=c(2.2,0,0.1,0.3),mar=c(0,2.5,0,0))
...
for (site in sites){
  ...
  axis(1, lwd = 0, lwd.ticks = 1, tck = 0.02,labels=F)
  axis(side=2,tick = F,line = -0.7,cex.axis=0.9)
  axis(2, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=F)
  mtext(paste0(def(site)),adj=0.97,side=3,line=-1.4,cex = 0.7)
...
axis(1, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=F)
#axis(1, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=T,cex.axis=0.9)
axis(side=1,tick = F,line = -0.7,cex.axis=0.9)
...
for (site in sites){
  ...
  for (model_name in models_avail){
    if(model_name == models_avail[1]){
      ...
      plot(1:xmax,t(growth[[site]][model_name])[1:xmax], col = get_model_colour(model_name) , ylim=c(-0.55, 1.55),type="l",ylab="",xaxt="n",lwd=1, yaxt="n")
    }else{
  ...
  axis(1, lwd = 0, lwd.ticks = 1, tck = 0.02,labels=F)
  axis(side=2,tick = F,line = -0.5,cex.axis=0.8)
  axis(2, lwd = 0, lwd.ticks = 1, tck = -0.02,labels=F)
  mtext(paste0(ghi(site)),adj=0.97,side=3,line=-1.4,cex = 0.7)
  ...
  if(site=="BCI"){
    ...
    legend(legend = c("BiomeE","BiomeEP","CABLE-POP","FATES","JULES-RED","LPJ-GUESS","EDv3","SEIB-DGVM","ORCHIDEE"),
         ...
         col = c(get_model_colour("BiomeE-Standalone"),
                 ...
                 get_model_colour("EDv3"),
                 get_model_colour("SEIB-DGVM"),
                 get_model_colour("ORCHIDEE")),"bottomright", ncol = 2, cex=0.5, box.lwd = 0)
...
mtext(expression("Woody biomass increment rate (kgCm"^-2~"yr"^-1~")"), side=2, line=-13.2, outer=TRUE,cex = 0.7)
#expression(log(nstems)~(nstems~ha^-1))
mtext(outer=TRUE,expression("Woody biomass mortality rate (% yr"^-1~")"),side=2, line=-1.6,cex=0.7)
mtext(outer=TRUE,"Year since disturbance",side=1, line=1,cex=0.7)
dev.off()
...
png(filename=paste0(Figs_dir,"FigS14_growth_rates_and_succession.png"),width=80,height=120,units = "mm",res=480)
#bitmap(file=paste0(Figs_dir,"/FigS14_growth_rates_and_succession.png"),width=80,height=120,units="mm",res =480,type="png16m")
...
```{r turnover_times_}
png(filename=paste0(Figs_dir,"FigS18_regrowth_speed_biomass_equilibrium.png"),width=80,height=120,units = "mm",res=480)
